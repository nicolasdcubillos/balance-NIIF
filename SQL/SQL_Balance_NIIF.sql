...
-- SELECT * FROM X_BALANCE_CC('5', 2023, 03) ORDER BY CODIGOCTA
...
--SELECT * FROM X_BALANCE_CC('5232092', 2023, 01) ORDER BY CODIGOCTA
--SELECT DEBITO - CREDITO FROM SALDCONTNI WHERE CODCC =  AND CODIGOCTA =  AND PERIODO = 11 AND ANO = 2022
--SELECT dbo.X_SALDOCUENTA_NIIF_CC('20230101', '110505', '5232092')
--SELECT * FROM SALDCONTNI WHERE CODIGOCTA LIKE '11%' AND CODCC = '5232092' AND ANO <= 2023 AND PERIODO <= 1
...
AS
	...
	(
		...
		WHERE TEMP.CODIGOCTA LIKE LTRIM(RTRIM(@cuenta)) + '%' AND TEMP.CODCC = @codcc AND TEMP.FECHA < @fecha
	)
	RETURN @valor
END
...
INSERT INTO @balance 
	...
	BEGIN
		...
		SELECT
			...
			FROM @balance
			WHERE ID = @CurrentRow
		INSERT INTO @balance SELECT CODIGOCTA, NOMBRECTA, @codcc, dbo.X_SALDOCUENTA_NIIF_CC(DATEFROMPARTS(CAST(@ano AS INT), CAST(@periodo AS INT), 1), CODIGOCTA, @codcc), 0, 0, 0 FROM X_CUENTASMAYORES(@cuentaBuscar)
	END;
RETURN
END;
